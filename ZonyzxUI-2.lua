...
local Settings = ZonyzxUI.Settings
local Flags = ZonyzxUI.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(ZonyzxUI.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then ZonyzxUI.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then ZonyzxUI.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then ZonyzxUI.Save["Theme"] = decode["Theme"] end
			end
		end
	end
	
	pcall(Save, "Zonyzx UI.json")
end
...
local Connections, Connection = {}, ZonyzxUI.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(ZonyzxUI.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = ZonyzxUI.Themes[ZonyzxUI.Save.Theme]
local PixelFont = Enum.Font.Arcade

local function AddEle(Name, Func)
	ZonyzxUI.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = ZonyzxUI.Elements[Ele](Instance, props, ...)
	return Element
end
...
-- /////////// --
function ZonyzxUI:GetIcon(index)
...
function ZonyzxUI:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	ZonyzxUI.Save.Theme = NewTheme
	SaveJson("Zonyzx UI.json", ZonyzxUI.Save)
	Theme = ZonyzxUI.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(ZonyzxUI.Instances, function(_,Val)
...
function ZonyzxUI:SetScale(NewScale)
	NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function ZonyzxUI:MakeWindow(Configs)
	local WTitle = Configs[1] or Configs.Name or Configs.Title or "Zonyzx UI"
	...
	local UISizeX, UISizeY = unpack(ZonyzxUI.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, ZonyzxUI.Save.TabSize, 1, -(TopBar.Size.Y.Offset + 46)),
	...
	local PlayerBar = Create("Frame", Components, {
		Size = UDim2.new(0, ZonyzxUI.Save.TabSize, 0, 42),
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			ZonyzxUI.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("Zonyzx UI.json", ZonyzxUI.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		ZonyzxUI.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("Zonyzx UI.json", ZonyzxUI.Save)
	end)
	...
	function Window:AddMinimizeButton(Configs)
		Configs = Configs or {}
		local BTitle = Configs[1] or Configs.Text or Configs.Title or "Menu"
		local BIcon = ZonyzxUI:GetIcon(Configs[2] or Configs.Icon or "menu")
		
		local Button = MakeDrag(InsertTheme(Create("ImageButton", ScreenGui, {
		...
		local IconLabel = InsertTheme(Create("ImageLabel", Button, {
		...
		local FloatBtn = {}
		function FloatBtn:SetText(NewText) TitleLabel.Text = NewText end
		function FloatBtn:SetIcon(NewIcon) IconLabel.Image = ZonyzxUI:GetIcon(NewIcon) end
		FloatBtn.Stroke = Stroke
		FloatBtn.Corner = Corner
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			ZonyzxUI.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(ZonyzxUI.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		local function Tabs()
			...
			table.foreach(ZonyzxUI.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(ZonyzxUI.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(ZonyzxUI.Options, {type = "Section", Name = SectionName, func = Section})
